...
本指南帮助已安装 OpenClaw 的用户快速将 LLM API 替换为国内版本，支持 MiniMax、智谱（GLM）和阿里云百炼三个主流国内 API。
...
阿里云百炼提供 OpenAI 兼容接口，可用于调用通义千问系列模型。
...
```
Model                                      Input      Ctx      Local Auth  Tags
bailian/qwen3-max-2026-01-23                text       262k     no    yes   configured
```
...
- 支持的 API: MiniMax 国内版、智谱 GLM、阿里云百炼
